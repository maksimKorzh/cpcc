...
board = [
    22,  0, 21, 23, 19, 21, 20, 22,    0,  0,  5,  5,  0,  0,  5,  0, 
    18, 18, 18,  0, 18, 18, 18, 18,    5,  5,  0,  0,  0,  0,  5,  5,
     0,  0, 20,  0,  0,  0,  0,  0,    5, 10, 15, 20, 20, 15, 10,  5,
     0,  0,  0, 18,  0,  0,  0,  0,    5, 10, 20, 30, 30, 20, 10,  5,    
     0,  0,  0,  9,  0, 13,  0,  0,    5, 10, 20, 30, 30, 20, 10,  5,
     0,  0,  0,  0,  0,  0,  0,  0,    5, 10, 15, 20, 20, 15, 10,  5,
     9,  9,  9,  0,  9,  9,  9,  9,    5,  5,  0,  0,  0,  0,  5,  5,
    14, 12,  0, 15, 11, 13, 12, 14,    0,  0,  5,  5,  0,  0,  5,  0
]
    ...
    if depth == 0:
        ...
        while index < 128:
            if (index & 0x88) == 0:
                if board[index]:
                    piece = board[index]
                    mat_score += piece_weights[piece & 15]
                    
                    if piece & 8:
                        pos_score += board[index + 8]
                    else:
                        pos_score -= board[index + 8]

            index += 1
    
        evaluation = mat_score + pos_score;
                    ...
                    while captured_piece == 0:
                        ...
                        if piece_type < 3:
                            if (target_square + step_vector + 1 & 0x80):
                                board[tagret_square] |= 7
                        
                        # recursive negamax search call
                        score = -search(24 - side, depth - 1)
                        ...
                        # brute force negamax search
                        if score > max_score:
                                max_score = score

                                # store best move so far
                                temp_src = source_square
                                temp_dst = target_square
...
best_score = search(white, 3)
print('score: %s | move: %s' % (best_score, notation[best_src] + notation[best_dst]))
print_board()
